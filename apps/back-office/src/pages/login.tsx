...
const REMEMBERED_USER_KEY = "trustoms-remember-user";
const SUPPORT_EMAIL = "support@trustoms.local";

const trustSignals = [
  {
    label: "Operations control",
    value: "4-eye",
    description: "Approvals, exceptions, and audit trails stay visible.",
    icon: ShieldCheck,
  },
  {
    label: "Portfolio oversight",
    value: "T+0",
    description: "Daily positions, cash, and client instructions in one place.",
    icon: WalletCards,
  },
  {
    label: "Risk posture",
    value: "Live",
    description: "Concentration, limit, and settlement checks before action.",
    icon: TrendingUp,
  },
];

const workspaceNotes = [
  "Maker-checker reviews are grouped by client, product, and value date.",
  "Corporate actions, allocations, and settlement tasks keep their evidence chain.",
  "Trust banking teams can move from exception triage to approval without losing context.",
];
...
export default function LoginPage() {
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      setNoteIndex((index) => (index + 1) % workspaceNotes.length);
    }, 6500);
  ...
  const handleForgotPassword = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (!trimmedEmail || !trimmedEmail.includes("@")) {
      setResetSent(false);
      setError("Enter the email address linked to your operations account.");
      return;
    }
    ...
    toast({
      ...
      description: "Your access desk will validate the request and follow up.",
    });
  };
        ...
        >
          ...
          <div className="relative max-w-2xl space-y-8">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                >
                  Trust banking command center
                </h1>
              </div>
            </div>

            <p className="max-w-xl text-base leading-7 text-slate-700 sm:text-lg">
              Secure operational access for trust product setup, client servicing,
              order oversight, approvals, and post-trade control.
            </p>
          ...
          <div className="relative mt-8 max-w-2xl rounded-lg border bg-card/90 p-5 shadow-sm">
            <div className="mb-4 flex items-center justify-between gap-4">
              <p className="text-sm font-medium text-muted-foreground">Workspace focus</p>
          ...
          <div className="w-full max-w-md">
            ...
            <div className="mb-5 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-sm text-muted-foreground">Back-office operations</p>
              </div>
            ...
            <Card className="border shadow-lg">
              <CardHeader className="space-y-2">
                ...
                <p className="text-sm leading-6 text-muted-foreground">
                  {forgotMode
                    ? "Submit your registered email address. The operations access desk will validate the request."
                    : "Use your Trust OMS operations account to continue."}
                </p>
              </CardHeader>
              <CardContent>
                {forgotMode ? (
                  <form className="space-y-5" onSubmit={handleForgotPassword} noValidate>
                    {(error || resetSent) && (
                      ...
                      >
                        ...
                        <span>
                          {error ||
                            "Recovery request captured. Check your inbox or contact the access desk for urgent unlocks."}
                        </span>
                      </div>
                    )}
