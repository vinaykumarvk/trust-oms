/**
 * Mid-Office Layout with Collapsible Sidebar Navigation
 *
 * Full sidebar layout for the TrustOMS mid-office Dealing Desk.
 * Features:
 ...
 * - Indigo/purple accent for visual distinction from front-office (teal) and back-office (slate)
 */
...
const SIDEBAR_COLLAPSED_KEY = "trustoms-mo-sidebar-collapsed";
...
function SkipLink() {
  return (
    <a
      href="#main-content"
      className="sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2 focus:z-[100] focus:rounded-md focus:bg-indigo-600 focus:px-4 focus:py-2 focus:text-white focus:shadow-lg"
    >
            ...
            <TooltipTrigger asChild>
              <div
                className={cn(
                  ...
                  hasSectionActive ? "text-indigo-600" : "text-muted-foreground"
                )}
              >
  ...
  const dashboardElement = collapsed ? (
    ...
    >
      <dashboardItem.icon
        className={cn(
          "h-4 w-4 shrink-0",
          dashActive ? "text-white" : "text-muted-foreground"
        )}
      />
      <span>Dashboard</span>
    </a>
  ...
  if (collapsed) {
    return (
      <div className="flex h-14 items-center justify-center border-b border-border px-2">
        <span className="text-lg font-bold text-indigo-600">T</span>
      </div>
    );
  }
...
function SidebarFooter({ collapsed }: { collapsed: boolean }) {
  ...
  let displayName = "MO User";
  let email = "";
  try {
    const stored = localStorage.getItem("trustoms-user");
    if (stored) {
      const user = JSON.parse(stored);
      displayName = user.name || user.email || "MO User";
      email = user.email || "";
    }
  } catch {
    // ignore
    ...
    <header className="sticky top-0 z-40 flex h-14 items-center gap-3 border-b border-border bg-background/95 backdrop-blur px-4 lg:hidden">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[9px] text-indigo-600 leading-tight">Mid Office</span>
        </div>
      </div>
    </header>
  );
...
export default function MidOfficeLayout() {
