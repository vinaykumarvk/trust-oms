/**
 * Front-Office Layout with Collapsible Sidebar Navigation
 *
 * Full sidebar layout for the TrustOMS front-office RM/Trader cockpit.
 * Features:
 ...
 * - Blue/teal accent for visual distinction from back-office
 */
...
const SIDEBAR_COLLAPSED_KEY = "trustoms-fo-sidebar-collapsed";
...
function SkipLink() {
  return (
    <a
      href="#main-content"
      className="sr-only focus:not-sr-only focus:fixed focus:top-2 focus:left-2 focus:z-[100] focus:rounded-md focus:bg-teal-600 focus:px-4 focus:py-2 focus:text-white focus:shadow-lg"
    >
            ...
            <TooltipTrigger asChild>
              <div
                className={cn(
                  ...
                  hasSectionActive ? "text-teal-600" : "text-muted-foreground"
                )}
              >
  ...
  if (collapsed) {
    return (
      <div className="flex h-14 items-center justify-center border-b border-border px-2">
        <span className="text-lg font-bold text-teal-600">T</span>
      </div>
    );
  }
...
function SidebarFooter({ collapsed }: { collapsed: boolean }) {
  ...
  let displayName = "RM User";
  let email = "";
  try {
    const stored = localStorage.getItem("trustoms-user");
    if (stored) {
      const user = JSON.parse(stored);
      displayName = user.name || user.email || "RM User";
      email = user.email || "";
    }
  } catch {
    // ignore
    ...
    <header className="sticky top-0 z-40 flex h-14 items-center gap-3 border-b border-border bg-background/95 backdrop-blur px-4 lg:hidden">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[9px] text-teal-600 leading-tight">Front Office</span>
        </div>
      </div>
    </header>
  );
