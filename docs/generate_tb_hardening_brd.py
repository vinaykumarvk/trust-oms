#!/usr/bin/env python3
"""
Generate TrustOMS Philippines — Trust Banking Hardening BRD v1.0
Covers confirmed implementation gaps: client portal security, messaging,
statements, SLA configuration, document storage, and degraded-mode persistence.
"""
...
meta_rows = [
    ("Document Title", "Trust Banking Hardening BRD v1.0"),
...
h2("4.3 service_request_documents")
p("Replaces the current string-array documents JSONB column. Each document is a separate row enabling per-document scan status and retention tracking.")
...
add_table(
    ...
    [
        ...
        ("retention_days", "INTEGER", "Y", "Days from SR resolution_date to soft-delete; 0 = no expiry", "2555"),
...
add_table(
    ...
    [
        ...
        ("override_reason", "TEXT", "N", "Reason text for manual override", "NULL"),
        ("created_at", "TIMESTAMPTZ", "Y", "—", "NOW()"),
    ],
    [1.8, 0.9, 0.7, 2.6, 0.6]
)
...
h3("FR-CP-SEC-001: Session-Derived Ownership Middleware")
...
bullet("AC-001.1: A middleware function validatePortalOwnership() extracts clientId from req.user.clientId.")
bullet("AC-001.2: If the route's :clientId parameter does not exactly match req.user.clientId, the middleware returns HTTP 403 with body { error: { code: 'FORBIDDEN', message: 'Access denied: resource does not belong to your account' } }.")
bullet("AC-001.3: If req.user.clientId is null or undefined (unauthenticated), middleware returns HTTP 401.")
bullet("AC-001.4: Ownership check failure is written to the audit log with fields: event='CLIENT_PORTAL_OWNERSHIP_VIOLATION', actor_id, attempted_client_id, actual_client_id, ip_address, path.")
bullet("AC-001.5: The middleware is applied to ALL client-portal routes that accept a :clientId parameter: service requests (list, create, detail, update, close, resubmit), statements (list, download), proposals (list, detail), elections, and documents.")
bullet("BR-001.1: SYSTEM_ADMIN and BO roles accessing client-portal routes on behalf of a client are excluded from ownership enforcement (they use back-office routes, not client-portal routes).")
...
h3("FR-STMT-002: Statement Download Endpoint")
...
bullet("AC-010.1: GET /api/v1/client-portal/statements/:clientId/:statementId/download returns the statement file.")
bullet("AC-010.2: Ownership check: :clientId must match session clientId (403 otherwise).")
bullet("AC-010.3: If file_reference points to an existing local file, the endpoint streams it with Content-Type: application/pdf and Content-Disposition: attachment; filename='statement-{period}.pdf'.")
bullet("AC-010.4: If file_reference is NULL or the file is missing, the endpoint generates a placeholder PDF containing: client legal name, portfolio ID, statement period, and a summary AUM figure. Returns HTTP 200 with the generated content.")
...
bullet("BR-010.1: Statements with delivery_status = FAILED still serve the placeholder PDF rather than returning an error to the client.")
...
h3("FR-SC-002: Configuration Update")
...
bullet("AC-013.1: PUT /api/v1/back-office/system-config/:key updates the value for the given key. Restricted to BO_HEAD and SYSTEM_ADMIN.")
...
h3("FR-DOCS-001: Document Upload (Client Portal)")
...
bullet("AC-015.1: POST /api/v1/client-portal/service-requests/:id/documents accepts multipart/form-data with field 'file'.")
bullet("AC-015.2: Ownership check: the service request must belong to the session client.")
bullet("AC-015.3: File is saved to uploads/sr-documents/{sr_id}/{uuid}-{filename} on the local filesystem.")
bullet("AC-015.4: A service_request_documents record is inserted with scan_status = PENDING, storage_reference set to the saved path.")
bullet("AC-015.5: Returns HTTP 201 with { id, document_name, file_size_bytes, scan_status, uploaded_at }.")
bullet("AC-015.6: Async scan job runs within 30 seconds of upload; marks CLEAN unless filename matches quarantine pattern (see BR below).")
bullet("BR-015.1: Files with extensions .exe, .bat, .sh, .cmd, .ps1 are immediately marked QUARANTINED without saving to disk.")
bullet("BR-015.2: File size limit is 20 MB. Files exceeding the limit return HTTP 413 with error code FILE_TOO_LARGE.")
bullet("BR-015.3: Accepted MIME types: application/pdf, image/jpeg, image/png, image/gif, application/vnd.ms-excel, application/vnd.openxmlformats-officedocument.spreadsheetml.sheet, application/msword, application/vnd.openxmlformats-officedocument.wordprocessingml.document.")
...
h3("FR-FEED-001: Feed Health Snapshot Persistence")
...
bullet("AC-019.1: degradedModeService.reportIncident() writes a feed_health_snapshots record after updating the in-memory registry.")
bullet("AC-019.2: On service module initialization (startup), degradedModeService loads the most recent feed_health_snapshots record per feed_name and populates the in-memory feedHealthRegistry.")
bullet("AC-019.3: If no snapshot exists for a feed at startup, the feed defaults to { status: 'UP', healthScore: 100, failureCount: 0 }.")
bullet("AC-019.4: Snapshot writes are asynchronous (fire-and-forget); a snapshot write failure does not block the incident report path.")
...
out_path = "docs/TrustBanking_Hardening_BRD_v1.docx"
doc.save(out_path)
